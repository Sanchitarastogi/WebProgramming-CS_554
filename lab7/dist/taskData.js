...
const getCollection = () => {
    return mongoHelper_1.MongoHelper.client.db("Rastogi_sanchita_CS554-lab7").collection('task');
};
...
async function createTask(title, description, hoursEstimated, completed, comments) {
    ...
    if (typeof description !== 'string' || description == '' || description == undefined)
        throw "Provide description ";
    if (typeof hoursEstimated !== 'number' || hoursEstimated == undefined || hoursEstimated <= 0)
        throw "Provide hoursEstimated ";
    ...
    if (!Array.isArray(comments) && ((typeof comments == "object") && (comments != ""))) {
        ...
        throw "Comments Needed";
    else if ((Array.isArray(comments) && comments.length > 0)) {
        for (var i = 0; i < comments.length; i++) {
            if (typeof comments[i].name !== "string" || comments[i].name == '' || comments[i].name == undefined)
                throw "Invalid comments";
            else if (typeof comments[i].comment !== "string" || comments[i].comment == '' || comments[i].comment == undefined)
                throw "Invalid comments";
            else {
                comments_S[i] = {
                    id: uuid(),
...
async function updateTask(id, updatedTask) {
    ...
    if (updatedTask.title) {
        if (typeof updatedTask.title !== "string" || updatedTask.title == "")
            throw "No title provided ";
        updatedTaskData.title = updatedTask.title;
    }
    if (updatedTask.description) {
        if (typeof updatedTask.description !== "string" || updatedTask.description == "")
            throw "No description provided ";
        updatedTaskData.description = updatedTask.description;
    }
    if (updatedTask.hoursEstimated) {
        if (typeof updatedTask.hoursEstimated !== "number" || updatedTask.hoursEstimated <= 0)
            throw "No estimated time provided ";
    ...
    if (updatedTask.comments) {
        if (!Array.isArray(updatedTask.comments) && ((typeof updatedTask.comments == "object") && (updatedTask.comments != ""))) {
            ...
            if (typeof updatedTask.comments[0].name !== "string" || updatedTask.comments[0].name == '' || updatedTask.comments[0].name == undefined)
                throw "Author is invalid for one or more comments";
            else if (typeof updatedTask.comments[0].comment !== "string" || updatedTask.comments[0].comment == '' || updatedTask.comments[0].comment == undefined)
                throw "Comment is invalid for one or more comments";
            else {
        ...
        else if (!updatedTask.comments)
            throw "Provide Comments";
        else if ((Array.isArray(updatedTask.comments) && updatedTask.comments.length > 0)) {
            updatedTaskData.comments = [];
            for (var i = 0; i < updatedTask.comments.length; i++) {
                if (typeof updatedTask.comments[i].name !== "string" || updatedTask.comments[i].name == '' || updatedTask.comments[i].name == undefined)
                    throw "Invalid comments";
                else if (typeof updatedTask.comments[i].comment !== "string" || updatedTask.comments[i].comment == '' || updatedTask.comments[i].comment == undefined)
                    throw "Invalid comments";
                else {
...
async function addCommentToTask(taskId, name, comment) {
    ...
    const addComment = await taskCollection.updateOne({ id: taskId }, { $push: { comments: commentCheck }
    });
