...
class Show extends Component {
   ...
   async getShow() {
      ...
      try {
         const response = await axios.get(
            `http://api.tvmaze.com/shows/${this.props.match.params.id}`   //get the show id 
         );
   ...
   render() {
      let body = null;
      const regex = /(<([^>]+)>)/gi;      //remove the extra html character
      let summary =
         this.state.data && this.state.data.summary.replace(regex, '');    //if there is data in state object than only do this
      ...
      return body;      //genre is an array we can map to it.
   }
}
